...
public class AnimMapBakerWindow : EditorWindow {
    ...
    private void Save(ref BakedData data)
    {
        ...
        AssetDatabase.SaveAssets();//资源写入内存
    ...
    private Texture2D SaveAsAsset(ref BakedData data)
    {
        ...
        AssetDatabase.CreateAsset(animMap, Path.Combine(folderPath, data.name + ".asset"));//在路径中创建一个新的资源
        return animMap;
    }
    ...
    private string CreateFolder()//创建文件夹
    {
        string folderPath = Path.Combine("Assets/" + path,  subPath);//连接两个路径字符串
        if (!AssetDatabase.IsValidFolder(folderPath))//给定文件夹的绝对路径，如果存在，则返回true，否则为false
        {
